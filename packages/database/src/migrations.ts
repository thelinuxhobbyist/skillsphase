...
CREATE INDEX IF NOT EXISTS "recommendations_user_id_idx" ON "recommendations" ("user_id");`,
  ...
  // 0009_primary_capability_outcome.sql
  `ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "primary_capability" text;
ALTER TABLE "projects" ADD COLUMN IF NOT EXISTS "outcome" text;`,

  // 0010_featured_project.sql
  `ALTER TABLE "projects" ADD COLUMN IF NOT EXISTS "featured" boolean DEFAULT false NOT NULL;`,

  // 0011_capabilities_evidence.sql
...
WHERE u.primary_capability IS NOT NULL
  ...
  // 0012_recommendation_privacy.sql
...
ALTER TABLE "recommendations" ALTER COLUMN "public_summary" SET NOT NULL;`,
  ...
  // 0013_homepage_messaging_refresh.sql
  `DELETE FROM homepage_sections WHERE type IN ('why_exists');`,

  // 0014_restore_jobs_applications.sql
  `DO $$ BEGIN
  CREATE TYPE "public"."job_status" AS ENUM('draft', 'published', 'closed');
EXCEPTION
...
EXCEPTION WHEN duplicate_object THEN null; END $$;
DO $$ BEGIN
  ALTER TABLE "jobs" ADD CONSTRAINT "jobs_created_by_user_id_users_id_fk"
    FOREIGN KEY ("created_by_user_id") REFERENCES "public"."users"("id") ON DELETE restrict ON UPDATE no action;
EXCEPTION WHEN duplicate_object THEN null; END $$;
DO $$ BEGIN
  ALTER TABLE "applications" ADD CONSTRAINT "applications_job_id_jobs_id_fk"
...
EXCEPTION WHEN duplicate_object THEN null; END $$;
DO $$ BEGIN
  ALTER TABLE "job_skills" ADD CONSTRAINT "job_skills_job_id_jobs_id_fk"
    FOREIGN KEY ("job_id") REFERENCES "public"."jobs"("id") ON DELETE cascade ON UPDATE no action;
EXCEPTION WHEN duplicate_object THEN null; END $$;
DO $$ BEGIN
  ALTER TABLE "job_skills" ADD CONSTRAINT "job_skills_skill_id_skills_id_fk"
    FOREIGN KEY ("skill_id") REFERENCES "public"."skills"("id") ON DELETE cascade ON UPDATE no action;
EXCEPTION WHEN duplicate_object THEN null; END $$;
